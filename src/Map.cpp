...
Map::Map(const unsigned int x, const unsigned int y, std::vector<std::vector<Area>> smap,
...
void Map::print(sf::RenderWindow& window) {
	...
	egg_sprite.setPosition(static_cast<float>(coordinate_egg.x * RESOLUTION_X_IMAGE),
	                       static_cast<float>( coordinate_egg.y * RESOLUTION_Y_IMAGE));
...
void Map::print(sf::RenderWindow& window, Coord tile, unsigned short los, bool loop) {
	...
	for (unsigned int i = y_los ; i-- ;) {
		...
		for (unsigned int j = x_los ; j-- ;) {
			tile.x %= x_size;
			sprites[map[tile.x][tile.y]].setPosition(static_cast<float>(tile.x * RESOLUTION_X_IMAGE),
...
void Map::print(Coord tile, sf::RenderWindow& window) {
	...
	sprites[map[tile.x][tile.y]].setPosition(static_cast<float>(tile.x * RESOLUTION_X_IMAGE),
	                                         static_cast<float>(tile.y * RESOLUTION_Y_IMAGE));
	window.draw(sprites[map[tile.x][tile.y]]);

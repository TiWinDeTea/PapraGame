...
static sf::Socket::Status
receiveWithTimeout(sf::UdpSocket& socket, char* data, size_t max_received_size, size_t& received_size,
                   sf::IpAddress& remote, unsigned short port, sf::Time timeout);
...
bool GameServer::getClients(sf::RenderWindow& window) {
	sf::Packet packet;
	packet.clear();
	packet << static_cast<unsigned int>(player.size()) << is_blind << los << loop << map_height << map_width
	       << game_speed << ressources << biome_path;
	...
	while (tmp_st > 0 && window.isOpen()) {
		...
		if (status == sf::Socket::Done && retrieved_data_size == 25 &&
		    std::string(retrieved_data) == "PapraGame ~ Game Request" && client_port == (PORT + 1)) {
...
void GameServer::launch(sf::RenderWindow& game_window) {
	...
	for (unsigned char i = 9 ; i-- ;) {
		if (!map_texture[i].loadFromFile(ressources + biome_path + map_textures_path[i] + FILETYPE)) {
			loading_success = loading_success &&
			                  map_texture[i].loadFromFile(ressources + "default/" + map_textures_path[i] + FILETYPE);
		}
	}

	loading_success = loading_success && egg_texture.loadFromFile(ressources + DUCK_PATH + TEXTURE_EGG + FILETYPE);
	loading_success =
			loading_success && explosion_texture.loadFromFile(ressources + DUCK_PATH + TEXTURE_EXPLOSION + FILETYPE);
	...
	for (unsigned int i = static_cast<int>(player.size()) ; i-- ;) {
		...
		for (unsigned int j = 4 ; j-- ;) {
			loading_success = loading_success && duck_texture[i][0][j].loadFromFile(
					ressources + duck_textures_path[j] + player_id + FILETYPE);
			loading_success = loading_success && duck_texture[i][1][j].loadFromFile(
					ressources + ducky_textures_path[j] + player_id + FILETYPE);
		}
	}
	...
	if (!loading_success) {
		...
		game_window.setView(
				sf::View(sf::FloatRect(0, 0, static_cast<float>(map_width * 32), static_cast<float>(map_height * 32))));
...
void GameServer::start(sf::RenderWindow& game_window) {
	...
	do {
			...
			for (unsigned int j(static_cast<unsigned int>(player.size())) ; j-- ;) {
				if (i != j) {
					if (player[i].getCoord() == player[j].getCoord() ||
					    ((player[i].getCoord() - player[i].getDirection() == player[j].getCoord()) &&
					     player[i].getDirection() != player[j].getDirection())) {
		...
		for (unsigned char w = 16 ; --w ;) {
			...
			while (game_window.pollEvent(event)) {
				if (event.type == sf::Event::Closed) {
					...
					if (event.key.code == player[player.size() - 1].keys[0] &&
					    (self_current_dir != DOWN || player[player.size() - 1].size() == 0)) {
						player_dir[player.size() - 1] = UP;
					} else if (event.key.code == player[player.size() - 1].keys[1] &&
					           (self_current_dir != UP || player[player.size() - 1].size() == 0)) {
						player_dir[player.size() - 1] = DOWN;
					} else if (event.key.code == player[player.size() - 1].keys[2] &&
					           (self_current_dir != RIGHT || player[player.size() - 1].size() == 0)) {
						player_dir[player.size() - 1] = LEFT;
					} else if (event.key.code == player[player.size() - 1].keys[3] &&
					           (self_current_dir != LEFT || player[player.size() - 1].size() == 0)) {
			...
			if (is_blind) {
				...
				for (unsigned char i = (unsigned char) player.size() ; i-- ;) {
					...
					for (unsigned char j = player[i].size() ; j-- ;) {
						game_map.print(game_window, player[i].duckies[j].getCoord(),
						               static_cast<unsigned short>(los / 2), loop);
					}
				}
		...
		for (unsigned char i((unsigned char) (clients.size())) ; i-- ;) {
			...
			packet >> dir_as_int;
			if (dir_as_int != NOPE &&
			    ((player_dir[i] + dir_as_int != 1 && player_dir[i] + dir_as_int != 5) || player[i].size() == 0)) {
...
std::vector<sf::Keyboard::Key> GameServer::loadKeys(std::string selected_player) {
	...
	if (keys_file.fail()) {
		...
		if (!key_loading_successful[0] || !key_loading_successful[1] || !key_loading_successful[2] ||
		    !key_loading_successful[3]) {
...
void GameClient::launch(sf::RenderWindow& game_window) {
	...
	if (broadcast.send("PapraGame ~ Game Request", 25, sf::IpAddress::Broadcast, PORT) != sf::Socket::Done) {
		...
		do {
			...
			if (receiveWithTimeout(broadcast, data, 28, received, sender, port, CLIENT_CONNECTION_REFRESH_RATE) ==
			    sf::Socket::Done) {
		...
		if (server.connect(sender, PORT, CLIENT_CONNECTION_TIMEOUT) != sf::Socket::Done) {
			...
			for (unsigned char i = 9 ; i-- ;) {
				loading_success =
						loading_success && map_texture[i].loadFromFile(path + map_textures_path[i] + FILETYPE);
			}

			loading_success = loading_success && egg_texture.loadFromFile(path + DUCK_PATH + TEXTURE_EGG + FILETYPE);
			loading_success =
					loading_success && explosion_texture.loadFromFile(path + DUCK_PATH + TEXTURE_EXPLOSION + FILETYPE);
			...
			for (unsigned int i = nbr_player ; i-- ;) {
				...
				for (unsigned int j = 4 ; j-- ;) {
					loading_success = loading_success && duck_texture[i][0][j].loadFromFile(
							path + duck_textures_path[j] + player_id + FILETYPE);
					loading_success = loading_success && duck_texture[i][1][j].loadFromFile(
							path + ducky_textures_path[j] + player_id + FILETYPE);
				}
			}
			...
			if (!loading_success) {
				...
				game_window.setSize(sf::Vector2u(map_width * 32, map_height * 32));
				game_window.setView(sf::View(
						sf::FloatRect(0, 0, static_cast<float>(map_width * 32), static_cast<float>(map_height * 32))));
				for (unsigned int i = nbr_player ; i-- ;) {
					player[i] = Duck(duck_texture[i][0], duck_texture[i][1], spawn[i], player_initial_dir[i],
					                 this->loadKeys("player 1"));
				}
				explosion_sprite.setTexture(explosion_texture);
		...
		while (game_window.pollEvent(event)) {
			if (event.type == sf::Event::Closed) {
				game_window.close();
			           ...
			           (elapsed_time.getElapsedTime().asSeconds() > 0.5f)) {
				end = true;
			}
		}

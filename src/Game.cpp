...
void Game::launch(sf::RenderWindow& game_window, std::string map_name) {
	...
	for (unsigned char i = 9 ; i-- ;) {
		if (!map_texture[i].loadFromFile(path + biome_path + map_textures_path[i] + FILETYPE)) {
			loading_success =
					loading_success && map_texture[i].loadFromFile(path + "default/" + map_textures_path[i] + FILETYPE);
		}
	}
	...
	loading_success =
			loading_success && explosion_texture.loadFromFile(path + ducks_path + TEXTURE_EXPLOSION + FILETYPE);
	...
	for (unsigned char i = player_number ; i-- ;) {
		...
		for (unsigned int j = 4 ; j-- ;) {
			loading_success = loading_success &&
			                  duck_texture[i][0][j].loadFromFile(path + duck_textures_path[j] + player_id + FILETYPE);
			loading_success = loading_success &&
			                  duck_texture[i][1][j].loadFromFile(path + ducky_textures_path[j] + player_id + FILETYPE);
		}
	}
	...
	if (!loading_success) {
		...
		game_window.setView(
				sf::View(sf::FloatRect(0, 0, static_cast<float>(pxl_length), static_cast<float>(pxl_height))));
		...
		pause_menu_sprite.setPosition(
				(static_cast<float>(pxl_length) - pause_menu_sprite.getGlobalBounds().width) / 2.0f,
				(static_cast<float>(pxl_height) - pause_menu_sprite.getGlobalBounds().height) / 2.0f);
			...
			if (event.type == sf::Event::Closed) {
				...
				for (unsigned char i = player_number ; i-- ;) {
					if (event.key.code == player[i].keys[0] &&
					    (player[i].getDirection() != DOWN || player[i].size() == 0)) {
						    player_dir[i] = UP;
					} else if (event.key.code == player[i].keys[1] &&
					           (player[i].getDirection() != UP || player[i].size() == 0)) {
						           player_dir[i] = DOWN;
					} else if (event.key.code == player[i].keys[2] &&
					           (player[i].getDirection() != RIGHT || player[i].size() == 0)) {
						           player_dir[i] = LEFT;
					} else if (event.key.code == player[i].keys[3] &&
					           (player[i].getDirection() != LEFT || player[i].size() == 0)) {
						           player_dir[i] = RIGHT;
					}
				}
		...
		if (is_blind) {
			...
			for (unsigned char i = player_number ; i-- ;) {
				...
				for (unsigned char j = player[i].size() ; j-- ;) {
					game_map.print(game_window, player[i].duckies[j].getCoord(), static_cast<unsigned short>(los / 2),
					               loop);
				}
			}
